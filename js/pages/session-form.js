    ...
    return `
      <div class="report-overlay">
        <div class="report-modal">
          <div id="session-report-box" class="card-soft">
            <div style="font-weight:800;color:#065f46;font-size:18px;">${
              data.sessionType === "quran" ? "تقرير حلقة قرآن" : "تقرير تربية إسلامية"
            }</div>
            <div style="font-size:13px;color:#6b7280;">المعلم: ${appState.settings.teacherName}</div>
            <div style="font-size:13px;color:#6b7280;">${data.dateAr || formatArDate(data.date)}</div>
            <hr />
            ${data.mode === "group" ? `
              <div style="font-weight:700;">المجموعة: ${data.groupName || ""}</div>
              <div style="font-size:12px;color:#6b7280;">عدد الحضور: ${data.participants.filter((p) => p.present !== false).length}</div>
            ` : `
              <div style="font-weight:700;">الطالب: ${data.studentName || ""}</div>
            `}
            <div style="margin-top:10px;font-size:13px;line-height:1.6;">
              ${data.sessionType === "quran" ? `
                ${data.quran?.hifz?.surah ? `<div>✨ تسميع: ${data.quran.hifz.surah} (${data.quran.hifz.from}-${data.quran.hifz.to})</div>` : ""}
                ${data.quran?.recent?.surah ? `<div>🔄 قريب: ${data.quran.recent.surah} (${data.quran.recent.from}-${data.quran.recent.to})</div>` : ""}
                ${data.quran?.distant?.surah ? `<div>🕰️ بعيد: ${data.quran.distant.surah} (${data.quran.distant.from}-${data.quran.distant.to})</div>` : ""}
                <div>⭐ التقييم العام: ${data.quran?.closing?.overall || 0}</div>
              ` : `
                ${data.islamic?.categories?.map((c) => `<div>📘 ${c.label || "درس"}</div>`).join("")}
                <div>⭐ التقييم السلوكي: ${data.overall || 0}</div>
              `}
            </div>
          </div>

          <div class="d-flex flex-wrap gap-2 mt-3">
            <button class="btn btn-success" onclick="sendReportWhatsApp()">📲 واتساب</button>
            <button class="btn btn-warning" onclick="exportElementAsImage(document.getElementById('session-report-box'), 'report.png')">🖼️ صورة</button>
            <button class="btn btn-light" onclick="editReportSession()">✏️ تعديل</button>
            <button class="btn btn-outline-dark" onclick="closeReport()">إغلاق</button>
          </div>
        </div>
      </div>
    `;
        ...
        <div class="card-soft">
          ...
          <div class="row g-3">
            <div class="col-12 col-lg-6">
              <div class="card-soft" style="background:#f0fdf4;">
                <div style="font-weight:700;color:#065f46;">قسم الحلقات الفردية</div>
                <div style="font-size:12px;color:#6b7280;" class="mb-2">اختر الطالب لتسجيل الجلسة</div>
            ...
            <div class="col-12 col-lg-6">
              <div class="card-soft" style="background:#fdfaf3;">
                <div style="font-weight:700;color:#92400e;">قسم الحلقات الجماعية</div>
                <div style="font-size:12px;color:#6b7280;" class="mb-2">اختر المجموعة لتسجيل جلسة جماعية</div>
    ...
    return `
      ...
      <div class="card-soft">
        ...
        ${form.sessionType === "quran" ? `
          ...
          <div class="card-soft mb-2" style="background:#fdfaf3;">
            <div style="font-weight:700;color:#92400e;margin-bottom:10px;">التدبر</div>
          ...
          <div class="card-soft mb-2">
            ...
            <div class="card-soft" style="background:#f9fafb;">
              <div style="font-weight:700;margin-bottom:10px;">واجب الحلقة القادمة</div>
        ...
        ` : `
          ...
          <div class="card-soft mb-2" style="background:#f0fdf4;">
            <div style="font-weight:700;color:#065f46;margin-bottom:10px;">التقييم السلوكي</div>
            <label class="form-label">الحضور والانتباه</label>
          ...
          <div class="card-soft mb-2" style="background:#fdfaf3;">
            <div style="font-weight:700;color:#92400e;margin-bottom:10px;">واجب الحصة القادمة</div>
